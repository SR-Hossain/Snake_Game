...
/*
   To upgrade: time, level(individual), use mouse cursor, connect to facebook
...
bool key_press()
{
    ...
    if (!(cs == 'a' or cs == 's' or cs == 'd' or cs == 'w' or cs == 'j'))
        cs = ch;
    else if (cs + ch == 234 or cs + ch == 197)
        cs = ch;
    else if (cs == 'j')
    {
        clscreen();
    ...
    if (totalboard[row][col] == campaign)
        return 0;
    else if (totalboard[row][col] == snakes)
        return 0;
    else if (totalboard[row][col] == fruits)
        fruit_eaten(row, col);
    else if (totalboard[row][col] == eagles)
        eagle_eaten(row, col);
    ...
    if (key == 1)
    {
        ...
        while (1)
        {
            ...
            if (points[stageh] == point)
            {
                ...
                if (survival)
                    print(br / 2 - 1, bc / 2 - 12, "Remaining Lives = "), cout << lives[0];
                else
                    print(br / 2 - 1, bc / 2 - 12, "Remaining Lives = "), cout << lives[stageh];
...
void eagle(int key)
{
    ...
    else if (key == draw)
    {
        for (int i = 0; i < eaglecount; i++)
        {
            if (totalboard[eaglerow[i]][eaglecol[i] - 2] == snakes)
            {
                lives[stageh]--;
                lives[0]--;
                continue;
    ...
    if (totalboard[snakerow[snakehead]][snakecol[snakehead]] == eagles)
        eagle_eaten(snakerow[snakehead], snakecol[snakehead]);
    snake(draw);
    print(br + 2, bc / 2, "    ");
}
...
void snake(int key)
{
    ...
    if (key == draw)
    {
        ...
        for (int i = snaketail; i <= snakehead; i++)
            print(snakerow[i], snakecol[i], i == snakehead ? "O" : i & 1 ? "ợ"
                                                                         : "ờ");
    }
    if (key == erase)
    {
...
void fruit(int key)
{
    ...
    {
        if (totalboard[fruitrow][fruitcol] == fruits)
            print(fruitrow, fruitcol, "erase");
    }
}
...
void board()
{
    ...
    {
        for (int i = 0; i < br; i++)
            print(i, -1, "🟩"), print(i, bc - 1, "🟩");
        for (int i = -1; i < bc; i += 2)
            print(0, i, "🟩"), print(br - 1, i, "🟩");

        for (int i = 0; i < br; i++)
            sp(i, 0, normal), sp(i, bc - 1, normal);
        for (int i = 0; i < bc; i++)
            sp(0, i, normal), sp(br - 1, i, normal);
    }
    else if (stageh == campaign)
    {
        for (int i = 0; i < br; i++)
            print(i, -1, "🧱"), print(i, bc - 1, "🧱"); // |
        for (int i = -1; i < bc; i += 2)
            print(0, i, "🧱"), print(br - 1, i, "🧱"); // _

        for (int i = 0; i < br; i++)
            sp(i, 0, campaign), sp(i, bc - 1, campaign);
        for (int i = 0; i < bc; i++)
            sp(0, i, campaign), sp(br - 1, i, campaign);
    }
    else if (stageh == tunnel)
    {
        ...
        for (int i = 11; i <= bc - 11; i += 2)
            print(7, i, "🧱"), print(br - 8, i, "🧱");

        for (int i = 1; i <= 4; i++)
            print(i, -1, "🧱"), print(i, bc - 1, "🧱"), print(br - 1 - i, -1, "🧱"), print(br - 1 - i, bc - 1, "🧱");
        for (int i = -1; i <= 8; i += 2)
            print(0, i, "🧱"), print(br - 1, i, "🧱"), print(0, bc - 2 - i, "🧱"), sp(0, bc - 1 - i, campaign), print(br - 1, bc - 2 - i, "🧱"), sp(br - 1, bc - 1 - i, campaign);
    }
    else if (stageh == apartment)
    {
        ...
        for (int i = -1; i <= bc; i += 2)
            print(7, i, "🧱"), i<20 ? print(br - 8, i, "🧱") : nothing(), i> 32 ? print(br - 8, i - 1, "🧱"), sp(br - 8, i, campaign) : 1;
        for (int i = 0; i < 8; i++)
            print(i, 13, "🧱");
...
void printarr(vector<string> vec)
{
    ...
    for (; it < veclen; it++)
    {
        ...
        if (veclen < 8 and it == pointpos)
            print(row, bc / 2 - vec[it].size() / 2 - 2, '>');
...
void control()
{
    ...
    else if (pointpos == 4 or pointpos == 0)
    {
        exit_the_game = 1;
        clscreen();
        return;
...
{
    cursor( row + winrow / 2 - 10, col + wincol / 2 - 20);
    if (str != "erase")
        cout << str;
    if (col == -1)
        col = 0;
...
{
    cursor( row + winrow / 2 - 10, col + wincol / 2 - 23);
    cout << str;
}
